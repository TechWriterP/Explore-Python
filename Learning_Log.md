...
## 📅 Day 32: Send Email (SMTP) & Manage Dates (Datetime)

### ✅ What I Learned Today
- [x] Using Python’s **smtplib** to connect to an email server  
- [x] Logging into SMTP securely with `.starttls()`  
- [x] Automating email sending with `sendmail()`  
- [x] Working with the **datetime module** (`datetime.now()`, `.weekday()`)  
- [x] Reading data from CSV with Pandas (`pd.read_csv`)  
- [x] Matching today’s date with birthday records  
- [x] Randomizing email templates and replacing placeholders with names  

### 🛠️ Project / Exercise
**Project Name:** Automated Birthday Wisher  
**Description:**  
- Store birthdays in a CSV file (`birthdays.csv`).  
- Program checks today’s date (month/day).  
- If a match is found, send an automated personalized birthday email.  
- Uses random template (`letter_1.txt`, `letter_2.txt`, etc.) for variety.  

**My Code:**  
[Link to my code](./Day32_BirthdayWisher/main.py)

### 💡 Challenges I Faced
- [x] Handling Gmail security settings (needed app-specific password)  
- [x] Date comparison mismatch due to leading zeros → solved by converting to integers  
- [x] Forgetting to strip `\n` when reading template files → fixed with `.replace("[NAME]", name)`  

### 🚀 Extensions / Improvements
- [ ] Integrate with an API (e.g., SendGrid) instead of plain SMTP  
- [ ] Add scheduling to run daily (Windows Task Scheduler or cron)  
- [ ] Store multiple emails per person (work/personal)  
- [ ] Extend project to send **weekly motivational quotes** instead of only birthdays  

### 📖 Resources
- Course: Day 32 – Send Email (smtplib) & Manage Dates (datetime)  
- Docs: [smtplib](https://docs.python.org/3/library/smtplib.html)  
- Docs: [datetime](https://docs.python.org/3/library/datetime.html)  
- Gmail App Passwords: [Google Support](https://support.google.com/accounts/answer/185833?hl=en)

# 🐍 100 Days of Code - Python Bootcamp (Angela Yu)

## 📅 Day 33: Working with APIs & JSON (ISS Overhead Notifier)

### ✅ What I Learned Today
- [x] What an **API** is and how to interact with it using `requests`  
- [x] Understanding **JSON responses** and converting them with `.json()`  
- [x] How to extract nested values from JSON (dict access)  
- [x] ISS Location API: `http://api.open-notify.org/iss-now.json`  
- [x] Sunrise-Sunset API: `https://api.sunrise-sunset.org/json`  
- [x] Working with **datetime** and comparing UTC times  
- [x] Automating an email alert when conditions are met (ISS overhead + nighttime)  

### 🛠️ Project / Exercise
**Project Name:** ISS Overhead Notifier  
**Description:**  
- Program checks if the ISS is close to your location (within ±5° lat/long).  
- Also checks if it’s currently dark (nighttime) at your location.  
- If both are true, it sends an email: *“Look up! The ISS is above you!”*  

**My Code:**  
[Link to my code](./Day33_ISSNotifier/main.py)

### 💡 Challenges I Faced
- [x] Remembering that Sunrise-Sunset API returns times in **UTC** → had to convert to local time  
- [x] Handling API request failures → used `response.raise_for_status()`  
- [x] Incorrect latitude/longitude comparison → fixed with absolute difference check  

### 🚀 Extensions / Improvements
- [ ] Run script automatically every 60 seconds using a scheduler  
- [ ] Add a Tkinter GUI with a live map of ISS position  
- [ ] Integrate with Twilio API to send SMS alerts  
- [ ] Extend to notify for other satellites or astronomy events  

### 📖 Resources
- Course: Day 33 – API Endpoints & JSON Data (ISS Overhead Notifier)  
